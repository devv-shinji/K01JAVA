...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		1-1] 객체저장: add()메소드
			...
			add(인덱스, 객체): 인덱스를 직접 부여하여 객체를 저장한다.단 인덱스를 건너뛰면 예외가 발생된다.
			
			size(): 컬렉션에 저장된 객체의 갯수를 반환한다.
				배열의 크기와 동일하다고 생각하면 된다.(length())
		 */
		list.add("소녀시대");//인덱스0
		list.add("빅뱅");
		list.add("트와이스");
		list.add(3, "워너원");
		//list.add(5, "소방차ㅋ");//에러발생. 인덱스는 건너뛸 수 없다.
		...
		/*
		1-5] 끼워넣기
			add(삽입할 인덱스, 객체): 해당 인덱스에 삽입하고, 기존데이터는 뒤로 밀리게 된다.
		 */
		...
		System.out.println(list.indexOf("방탄소년단"));
