...
//		LinkedList<Student> list2 = new LinkedList<Student>();
		ArrayList<Student> list2 = new ArrayList<Student>();//ArrayList로도 가능
		
		//개발자가 정의한 인스턴스 생성(외부패키지에 저장이 되어있음)
		Student st1 = new Student("정우성", 10, "2018");
		Student st2 = new Student("원빈", 20, "2017");
		Student st3 = new Student("장동건", 30, "2016");
		Student st4 = new Student("현빈", 40, "2015");
		...
		//인덱스를 통한 삭제: 삭제 성공 시 해당객체가 반환된다.
		System.out.println("삭제된 객체의 이름:"+ list2.remove(2).name);//.name이 없으면 참조값이 출력된다.
		//인스턴스 참조값을 통한 삭제: 삭제 성공 시 boolean이 반환된다.
		System.out.println(list2.remove(st1));
