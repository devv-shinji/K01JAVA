...
x, y 좌표(그림판좌표)는 0 ~ 100 사이의 값을 가진다. 
...
class Rectangle{
    ...
    private int ulx, uly;//좌상단(upper left x, upper left y)
		...
		*/
   		...
   		if (ulx>=lrx || uly>=lry) { //우하단좌표값이 무조건 더 커야한다. 그림판식 좌표이니까.
   	...
   	public void showArea()
   	{
   		...
   		int xLength = lrx = ulx;
   		int yLenght = lry = uly;
   		System.out.println("사각형의 넓이"+ (xLength*yLenght));
   	}  	
}
...
// 		rec.lry=10;
		...
		Rectangle rec3 = new Rectangle(10,10,1,1);//현재 사용한 그림판식 좌표는 우측하단 좌표가 무조건 더 커야한다.
		rec3.showArea();
	}
