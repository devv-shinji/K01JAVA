package ex12inheritance;

public class E07RideAndLoad02 {

	public static void main(String[] args) {

		System.out.println("Child형 참조변수로 Child객체 참조");
		DeChild child = new DeChild("율곡이이", 49, "00학번");
		
		/*
		오버라이딩 처리된 메소드이므로 참조변수에 상관없이
		무조건 자식쪽 메소드가 호출됨
		 */
		child.excercise();
		child.sleep();
		child.printParent();
		
		child.study();//자식에서 새롭게 확장한 멤버메소드
		child.walk();//오버로딩-부모쪽
		child.walk(25);//오버로딩-자식쪽
		
		//정적메소드이기 때문에 클래스명으로 호출함.
		DeChild.staticMethod();//스태틱변수or메소드는 앞에 클래스명붙여서 호출이가능하다.
		
		////////////////////////////////////////////
		
		System.out.println("Parent형 참조변수로 Child객체 참조");
		DeParent parent = new DeChild("퇴계이황", 35, "99학번");
		
		/*
		오버라이딩 처리된 메소드이므로 참조변수에 상관없이
		무조건 자식쪽 메소드가 호출됨.
		참조변수의 영향을 받지 않는다.
		 */
		parent.excercise();//오버라이딩되면 무조건 하위클래스 호출
		parent.sleep();//오버라이딩되면 무조건 하위클래스 호출
		parent.printParent();//오버라이딩되면 무조건 하위클래스 호출
		
		//parent.study();//자식에서 확장한 메소드 - 접근불가
		parent.walk();
		//parent.walk(20);//자식에서 확장한 메소드 - 접근불가
		
		//정적메소드이기 때문에 클래스명으로 호출함.
		DeParent.staticMethod();
		
		
	}

}
