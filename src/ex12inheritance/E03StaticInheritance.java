...
/*
static(정적) 타입의 멤버도 상속이 되고, 하위클래스에서 접근할 때는
 ...
 */
class AdderFriend extends Adder {//자식클래스
	public void friendsAdd(int num) {
		val += num;//아래처럼도 가능.
		Adder.val += num;
	}
	public void showVal() {
		System.out.println("val="+ val);//아래처럼도 가능.
...
public class E03StaticInheritance {
	...
	public static void main(String[] args) {
		...
		ad.add(1);//1+1
		adFriend.friendsAdd(3);//3+3
		Adder.val += 5;
		AdderFriend.val += 7;
		
		adFriend.showVal(); //1+3+5+7의 결과: 16이 출력됨 => 2+6+5+7=20(중간예시 주석처리 풀어서 두번씩 계산된다)
