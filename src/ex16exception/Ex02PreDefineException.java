...
public class Ex02PreDefineException {
	...
	public static void main(String[] args) {
		...
		try {
			...
			printStackTrace(): 예외가 발생할 때의 에러메세지를 그대로 콘솔에 출력해줌.
							개발시 훨씬 더 사용빈도 높음
			 */
			System.out.println("예외메세지:"+ e.getMessage());//getMessage는 간략히 메세지보여줌
			e.printStackTrace();
		}
		...
		try {
			...
			System.out.println("당신의 10년후 나이는:"+ ageAfter10);//실행안되고 catch로 넘어감
		} 
		...
		System.out.println("toDay="+ toDay);//toString()의 도움을 받아 문구로 출력.
		/* toDay 참조변수가 가지고 있는 null값을 toSting()메소드를 통해 
		...
		try {
			...
			//위에서 Date형으로 선언만하고, 객체 생성을 하지 않았기 때문에 하위메소드인 getTime을 호출할 수 없다.
		} 
		...
		try {
			...
			//Object와 strObject는 상속관계가 아니므로, 다운캐스팅이 불가능하다.
		} 
	...
	/*
	String 객체가 매개변수로 전달되면서 Object로 자동형변환(업캐스팅)되기 때문에,
	여기서는 상속관계가 확인되어 true를 반환할 수 있다.
	 */
