...
class ShowThread extends Thread {
	...
	해당 메소드는 직접 호출하면 안되고, start()메소드를 통해 간접적으로 호출해야 한다.
	만약 직접 호출하면 단순한 실행만 될 뿐 쓰레드가 생성되지 않으므로 주의해야 한다.
	 */
	@Override
	public void run() {//run()은 main함수처럼 자동으로 호출되므로 직접 불러 사용 하지말자.
		for(int i=1; i<=100; i++) {
...
//		st1.run(); //run()메소드를 호출하면 쓰레드가 생성(실행)되지 않는다. 일반 메소드처럼 작동함..
//		st2.run();
	}
}
