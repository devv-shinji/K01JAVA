...
class MessageSendingThread extends Thread {
	...
	public void run() {
		for(int i=1; i<=1000; i++) {
			...
			/*
			기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만, sleep()메소드를 사용하면
			실행중 블럭(Blocked)상태로 전환되어 우선순위가 낮은 쓰레드도 가끔 실행될 수 있는 상황이 생기게 된다.
			 */
			try {
				//0.001초동안 블럭상태로 만듬
				sleep(1);
...
public class Ex02Priority {
	public static void main(String[] args) {
		...
		/*
		1단계
			: 우선순위를 별도로 지정하지 않은 상태로 쓰레드를 생성하면 동일한 우선순위 5를 부여받게 된다. 
